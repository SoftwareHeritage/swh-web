...
class ServiceTestCase(test_app.SWHApiTestCase):
    ...
    @istest
    def lookup_directory_through_revision_empty(self, mock_lookup_rev):
        # given
        mock_lookup_rev.return_value = None

        # when
        rev_id, actual_dir = service.lookup_directory_through_revision(
            {'sha1_git': 'sha1'}, 'some/path', 100)

        # then
        self.assertIsNone(rev_id)
        self.assertIsNone(actual_dir)

        mock_lookup_rev.assert_called_once_with({'sha1_git': 'sha1'}, 100)

    @patch('swh.web.ui.service.lookup_revision_through')
    @patch('swh.web.ui.service.lookup_directory_with_revision')
    @istest
    def lookup_directory_through_revision(self,
                                          mock_lookup_dir,
                                          mock_lookup_rev):
        # given
        mock_lookup_rev.return_value = {'id': 'rev-id'}
        mock_lookup_dir.return_value = {'type': 'dir', 'content': []}

        # when
        rev_id, actual_dir = service.lookup_directory_through_revision(
            {'sha1_git': 'yet-another-sha1'}, 'some/path', 100)

        # then
        self.assertEquals(rev_id, 'rev-id')
        self.assertEquals(actual_dir, {'type': 'dir', 'content': []})

        mock_lookup_rev.assert_called_once_with(
            {'sha1_git': 'yet-another-sha1'}, 100)
        mock_lookup_dir.assert_called_with('rev-id', 'some/path')
